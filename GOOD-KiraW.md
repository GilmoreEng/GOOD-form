...
|Culture/Community|
|TDX/Service| Met with Luis Torrez for scale testing of Ansys CFD simulation. Scaled to 4 nodes on wendian. Account approved. Meeting set to review submission process. Jetstream2 windows build for BPX team, Mount 100GB drive on D:, mount vault.mines.edu/adm to install ArcPro and Ansys. by fixing IP of vault for TCP/UDP 111 2049 ports on Jetstream2 interface. Test Ansys connection to license server by openning IP of plicsvr-ansys.mines.edu and openning ports using security groups. Got quote for 64CPU windows 2022 server license for $210 for 8-month. No response from team. Closed all resouces on Jetstream and destroy storage, ArcPro image remains shevled.
|On-prem Cluster| Fix Ansys multi-node on Wendian. Mio is still command-line only.
|Cloud/HPC IaaS| 
...
|Website| Website Docs out of date need to update after workshop. See above. Also update SciVis offering. Correct, WARNINGS, remove old files after reviewing content, and fix tables for in interal RC management gitlab repo.
